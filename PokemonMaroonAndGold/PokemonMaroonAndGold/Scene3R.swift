...
//
//  Created by Junshu Liu on 12/5/17.
...
class Scene3R: SKScene, SKPhysicsContactDelegate {
    ...
    func createSceneObjects() {
        ...
        let scene = SKTexture(imageNamed: "Scene3Rc.png")
        ...
        character.position = CGPoint(x: 20, y: frame.height-20)
    ...
    func createEdgeBottom() {
        let bottom = SKSpriteNode(color: SKColor.blue, size: CGSize(width: 30, height: frame.height))
        bottom.position = CGPoint(x: frame.width+30, y: frame.midY)
    ...
    func createEdgeMU() {
        let MU = SKSpriteNode(color: SKColor.blue, size: CGSize(width: 30, height: 30))
        MU.position = CGPoint(x: 200, y: 50)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if contact.bodyA.node?.name == "character" || contact.bodyB.node?.name == "MU" {
            let goToNextScene = SKTransition.fade(withDuration: 0.5)
            view?.presentScene(controller.titleScreen, transition: goToNextScene)
        }
    }
}
